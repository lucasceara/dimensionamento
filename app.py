...
def obter_pc_por_faixa(xk_centro, w, t, Ne, h_total, wander_std):
    faixa_largura = 25.4
    ...
    if Ne == 1 or h_total >= t - w:
        ...
        weq = w + h_total
        desloc = t / 2
        posicoes = [xk_centro - desloc, xk_centro + desloc]
...
for nome, props in aeronaves_dict.items():
    ...
    if a > 0:
        ...
        xk = next(d['xk_centro'] for d in modelo['aeronaves_adicionais'] if d['nome'] == nome) if 'aeronaves_adicionais' in modelo else 300
