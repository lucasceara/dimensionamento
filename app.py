...
def calcular_C(ev):
    ...
    ev_min = 1e-5  # Aumentado para maior estabilidade
    ev = max(ev, ev_min)
    
    if ev >= 1.765e-3:
        C = (0.00414131 / ev) ** 8.1
    else:
        # Garantir que o termo seja positivo e estável
        termo = max(185.19 * ev, 1e-4)  # Ignorar o termo -0.1638 para pequenas deformações
        log_C = termo ** (-0.60586)
        C = 10 ** log_C
    
    # Definir limites mais restritivos para C
    C_max = 1e8  # Reduzido para evitar valores extremos
    C_min = 1e-1  # Aumentado para evitar CDFs muito pequenos
...
if st.button("Gerar gráfico CDF acumulado", type="primary"):
    ...
    # Plotar cdf_total mesmo que seja pequeno, para verificar
