...
Temperatura padrão para o MR: **25°C**  
Correção térmica: \( E_T = E_{25} \cdot e^{-0.08 \cdot (T - 25)} \)

---

### 🎯 Coeficientes de Variação (COV)
Indica incerteza (%) de cada variável.  
Para análise determinística, use COV = 0.

---

### ✈️ Desvio-padrão do Wander
Variação lateral da posição da aeronave.  
Valor padrão: **77,3 cm (FAA)**.
""")
...
Tc = st.number_input("Temp. média anual (°C)", value=27.0)
...
"
                    f"- MR original: {revest_MR:.2f} MPa | Deformação: {ev_original:.6f} mm/mm  
"
                    f"- MR corrigido: {entrada_corrigida[2]:.2f} MPa | Deformação: {ev_corrigida:.6f} mm/mm  
"
                    f"- Deformação confiável ({confianca:.0f}%): {ev_conf:.6f} mm/mm")

    st.subheader("Distribuição do Dano Acumulado (CDF)")
    fig, ax = plt.subplots(figsize=(10, 5))
    ax.plot(fx, cdf_total, color="black", label="CDF total")
    ax.axhline(1, linestyle="--", color="red", label="Limite CDF = 1")
