...
def calcular_C(ev):
    ...
    ev_min = 1e-6  # Valor mínimo para deformação
    ev = max(ev, ev_min)
    
    if ev >= 1.765e-3:
        C = (0.00414131 / ev) ** 8.1
    else:
        # Garantir que o termo seja positivo e evitar valores extremos
        termo = max(-0.1638 + 185.19 * ev, 1e-6)
        log_C = termo ** (-0.60586)
        C = 10 ** log_C
    
    # Limitar C para evitar valores extremos
    C_max = 1e10  # Valor máximo para C
    C_min = 1e-2  # Valor mínimo para C
...
def obter_pc_por_faixa(aeronave_nome, h_total, dados_aeronaves, wander_std=77.3):
    ...
    for aero in dados_aeronaves:
        if aero['nome'] == aeronave_nome:
            ...
            posicoes_pneus_sim “‘trico = posicoes_pneus + [-pos for pos in posicoes_pneus]
...
if st.button("Gerar gráfico CDF acumulado", type="primary"):
    ...
    for nome, (dec, cres) in aeronave_inputs.items():
        if dec > 0:
            ...
            if np.all(np.isfinite(cdf)) and not np.any(np.isnan(cdf)):
                cdf_total += cdf
                valid_cdf_count += 1
                ax.plot(fx, cdf, label=f"{nome}")
            else:
                st.warning(f"⚠️ CDF para {nome} contém valores inválidos e foi ignorado na soma do CDF Total.")

    if valid_cdf_count > 0:
        ax.plot(fx_ref, cdf_total, color='black', linestyle='--', linewidth=3, label='CDF Total (soma)')
    else:
        st.error("Nenhum CDF válido foi calculado. Verifique os valores de entrada, especialmente as deformações muito baixas.")
